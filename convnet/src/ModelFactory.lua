...
function ModelFactory.generate_overfeat(opt, nb_class)
    ...
    model:add(lf.ReLU(opt))
    model:add(lf.SpatialPooling(opt, 3, 3, 3, 3))
    model:add(lf.SpatialConvolution(opt, 96, 256, 7, 7, 1, 1))
    model:add(lf.ReLU(opt))
    model:add(lf.SpatialPooling(opt, 2, 2, 2, 2))
...
function ModelFactory.generate_small(opt, nb_class)
    local model = nn.Sequential()
    model:add(nn.SpatialConvolution(3, 4, 3, 3, 1, 1, 1, 1))
    model:add(nn.ReLU(true))
    model:add(nn.SpatialMaxPooling(2, 2, 2, 2))
    model:add(nn.SpatialConvolution(4, 8, 3, 3, 1, 1, 1, 1))
    model:add(nn.ReLU(true))
    model:add(nn.SpatialMaxPooling(2, 2, 2, 2))
    model:add(nn.SpatialConvolution(8, 8, 3, 3, 1, 1, 1, 1))
    model:add(nn.ReLU(true))
